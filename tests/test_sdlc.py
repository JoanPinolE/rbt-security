"""
tests/test_sdlc.py  —  VERSION INTEGRADA
=========================================
Todos los tests del proyecto organizados por fase del SDLC.
Cada suite cubre una fase y referencia los archivos especializados.
...
Total: 6 fases · 71 tests · 1 archivo
...
Por fase:
    ...
    pytest tests/test_sdlc.py -v -k "fase3"   # sin API (mocks)
    pytest tests/test_sdlc.py -v -k "not fase6"

Sin API (fases 2 y 3 solamente):
    pytest tests/test_sdlc.py::TestFase2Diseno -v
    pytest tests/test_sdlc.py::TestFase3Desarrollo -v
"""
...
class TestFase3Desarrollo:
    ...
    def test_UT05_headless_ua_scores_15(self):
        ...
        assert analyze_behavioral_ai(req) == 15.0
...
class TestFase6Operaciones:
    ...
    def test_OP04_system_recovers_after_spike(self):
        ...
        def req():
            try:
                r = legit().get(f"{BASE}/api/data", timeout=5)
        ...
        assert conn_errors == 0, f"{conn_errors} peticiones fallaron con error de conexion"
